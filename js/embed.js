...
ajax.onreadystatechange=function(){
    if (ajax.readyState==4 && ajax.status==200)
    {
        ...
        encuss_sub_replybox.style.marginLeft="65px";
...
function createCommentNode(info)
{
    ...
    <span class="encuss-comment-content">\
    </span>\
    <span class="encuss-comment-time">\
</span>\
<span class="encuss-comment-action">\
    ...
    ele.getElementsByClassName("encuss-comment-content")[0].innerText=info.content_reply+ info.content;
...
function submitComment(curObj)
{
    ...
    if(postComment(parent_id,textarea.value))
        textarea.value="";

}
function postComment(parent_id,content)
{
    var ajax=new XMLHttpRequest();
    ...
    ajax.onreadystatechange=function(){
        if (ajax.readyState==4 && ajax.status==200)
        {
            ...
            else
            {
                ...
                window.scrollTo(0,new_comment.offsetTop);
